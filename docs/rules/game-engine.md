...
`competitor_id`, `player_id`, `segment`, `multiplier`, `score`, `input_method`, plus optional `x`/`y` (see `buildDartThrownEvent` in `lib/features/game/domain/usecases/game_use_case_helpers.dart`). `x`/`y` are the auto-scorer's normalised canonical impact position (#571, canonical scoring frame: origin = centre, radius 1.0 at the double ring, **5/20 calibration wire at top** so segment 20's centre is ~9° clockwise of vertical; the heatmap widget rotates only the *display* via `kBoardDisplayRotation` to show a standard "20 at top" board, #697 — stored values stay in the wire-at-top frame) — **omitted when null** (manual entry, noise, pre-#571 events) so old events parse unchanged. The same values are written to `dart_throws.x`/`.y`. The use cases read them off the `DartThrow` they already receive (the notifier sets them). An undo deletes the dart, and re-entry is a fresh manual throw → position stays null by design. A **correction** rewinds the target *and the darts after it*, then re-throws them: the corrected dart keeps its `input_method` but loses its position (the detected x/y no longer matches the segment the user settled on), while every untouched tail dart is restored with the `input_method` **and** `x`/`y` it was recorded with. Both replacements are tagged `rethrown: true` — the original events stay in the log, so anything counting *what the camera detected* (e.g. `correlateCameraDarts`) must skip replacements or a corrected session reads as misaligned — dropping the provenance re-recorded them as positionless manual darts, which silently erased them from the heatmap and made a second correction in the same turn skip the auto-scorer capture (#788, `CorrectDartUseCase`). No `turn_number`, no `dart_number` — reconstruct turn grouping via `TurnStarted`/`TurnEnded` event boundaries if needed.
