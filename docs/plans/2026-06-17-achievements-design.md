...
- **SI-5** ✅ (#526) — `AchievementsPage` + "Succès" entry (trophy header action) in the stats screen. Notes: titles/descriptions now **localized** (34 ARB keys × 7 langs, resolved via a `switch (id)` helper — supersedes the "Hardcoded EN here" note in §3.5); the unlock date comes from a new `AchievementRepository.watchUnlockedDetails` (id→unlockedAt map); page watches `achievementMetricsProvider` + `unlockedAchievementsProvider`.
- **SI-6** ✅ (#527) — Unlock notification (toast + queue) + `SoundCue.achievementUnlock` (enum + asset + mapping). Notes: global `AchievementNotificationHost` mounted via `MaterialApp.router`'s `builder` + a `scaffoldMessengerKey` (replaces the SI-4 activation placeholder); sound played **once per batch**; `assets/sounds/achievement.wav` is a **placeholder** (copy of cricketTripleMark) tracked by #567 for a dedicated sound. **Epic #521 complete.**
