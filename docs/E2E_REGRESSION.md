...
| `@screenshots` | **generator, not a regression spec** — `capture-screenshots.spec.ts` drives the sim build to produce Play-Store / landing-page screenshots (light + dark). Needs the sim server on `:6780`; run only via `npx playwright test --grep @screenshots`. Excluded from regression slices. |
