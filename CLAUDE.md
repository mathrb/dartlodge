...
**Game config is edited in a bottom sheet, not a page:** X01/Cricket in/out strategy, legs-to-win, and starting score are set via the config-summary chip → `GameConfigPanel` bottom sheet on the player-selection screen (`APPLY` commits). `GameConfigPage` exists but is unwired, and the X01 "Custom" variant tile is disabled. The `LEGS TO WIN` `ConfigStepperWidget` +/- icon buttons have no `semanticLabel` (#666); the X01 config-summary chip omits `legsToWin` (#667). (E2E driving notes: `docs/E2E_REGRESSION.md` § Authoring specs.)
