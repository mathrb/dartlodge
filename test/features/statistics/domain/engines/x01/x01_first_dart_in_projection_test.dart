import 'package:flutter_test/flutter_test.dart';
import 'package:dart_lodge/core/utils/constants.dart';
import 'package:dart_lodge/features/game/domain/entities/game_event.dart';
import 'package:dart_lodge/features/statistics/domain/engines/projection_engine.dart';
import 'package:dart_lodge/features/statistics/domain/engines/x01/x01_first_dart_in_projection.dart';

GameEvent _makeEvent(
  String type,
  Map<String, dynamic> payload, {
  int seq = 1,
}) =>
    GameEvent(
      eventId: 'evt-$seq-$type',
      gameId: 'game-1',
      eventType: type,
      localSequence: seq,
      occurredAt: DateTime(2024),
      payload: payload,
      synced: false,
      actorId: 'p1',
      source: EventSource.client,
    );

ProjectionContext _makeContext({
  String playerId = 'p1',
  String inStrategy = 'double',
}) =>
    ProjectionContext(
      playerId: playerId,
      gameType: GameType.x01,
      inStrategy: inStrategy,
      outStrategy: 'double',
      playerIds: ['p1', 'p2'],
    );

void main() {
  late X01FirstDartInProjection engine;

  setUp(() {
    engine = X01FirstDartInProjection();
  });

  // ── Category A ─────────────────────────────────────────────────────────────

  test('A1 — init with double in produces zero counters', () {
    engine.init(_makeContext());
    final s = engine.snapshot();
    expect(s['firstDartInSuccessRate'], isNull);
    expect(s['inAttempts'], 0);
    expect(s['inSuccesses'], 0);
  });

  test('A1 — init with straight in returns empty snapshot', () {
    engine.init(_makeContext(inStrategy: 'straight'));
    expect(engine.snapshot(), isEmpty);
  });

  test('A4 — snapshot deterministic after init', () {
    engine.init(_makeContext());
    expect(engine.snapshot(), engine.snapshot());
  });

  // ── Category B ─────────────────────────────────────────────────────────────

  test('B1 — TurnStarted then double dart succeeds (double in)', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('B1 — TurnStarted then single dart fails (double in)', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 0);
  });

  test('B1 — master in: triple dart succeeds', () {
    engine.init(_makeContext(inStrategy: 'master'));
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'T20', 'score': 60}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('B1 — master in: single dart fails', () {
    engine.init(_makeContext(inStrategy: 'master'));
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 0);
  });

  test('B2 — DartThrown without preceding TurnStarted is ignored', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}));
    expect(engine.snapshot()['inAttempts'], 0);
  });

  test('B2 — second dart in same turn is ignored (only first dart counts)', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 2));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 3));
    expect(engine.snapshot()['inAttempts'], 1);
  });

  test('B2 — other player TurnStarted does not set awaitingFirstDart for p1', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p2'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 0);
  });

  // ── Category C ─────────────────────────────────────────────────────────────

  test('C1 — success rate calculates correctly', () {
    engine.init(_makeContext());
    // Turn 1: miss
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 2));
    engine.apply(_makeEvent('LegCompleted', {'winner_player_id': 'p2'}, seq: 3));
    // New leg: Turn 2: hit double
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 4));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 5));
    // 1 success / 2 attempts = 50%
    expect(engine.snapshot()['firstDartInSuccessRate'], closeTo(50.0, 0.001));
    expect(engine.snapshot()['inAttempts'], 2);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('C1 — once in, subsequent turns in same leg do not count attempts', () {
    engine.init(_makeContext());
    // First turn: succeed
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    // Second turn: should NOT count
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 3));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 4));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('C3 — parallel engines converge', () {
    final e2 = X01FirstDartInProjection();
    engine.init(_makeContext());
    e2.init(_makeContext());
    final events = [
      _makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1),
      _makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2),
    ];
    for (final e in events) {
      engine.apply(e);
      e2.apply(e);
    }
    expect(engine.snapshot(), e2.snapshot());
  });

  // ── Category D ─────────────────────────────────────────────────────────────

  test('D1 — reset(turn) does not clear counters', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    engine.reset(ProjectionScope.turn);
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('D2 — reset(leg) does not clear counters (cumulative)', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    engine.reset(ProjectionScope.leg);
    expect(engine.snapshot()['inAttempts'], 1);
  });

  // ── Category E ─────────────────────────────────────────────────────────────

  test('E1 — replay yields identical snapshot', () {
    engine.init(_makeContext());
    final events = [
      _makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1),
      _makeEvent('DartThrown', {'player_id': 'p1', 'segment': '20', 'score': 20}, seq: 2),
      _makeEvent('LegCompleted', {'winner_player_id': 'p2'}, seq: 3),
      _makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 4),
      _makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 5),
    ];
    for (final e in events) engine.apply(e);
    final first = engine.snapshot();
    engine.init(_makeContext());
    for (final e in events) engine.apply(e);
    expect(engine.snapshot(), first);
  });

  // ── Category F ─────────────────────────────────────────────────────────────

  test('F1 — partial stream without LegCompleted is valid', () {
    engine.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    expect(engine.snapshot()['inAttempts'], 1);
    expect(engine.snapshot()['inSuccesses'], 1);
  });

  test('Rat1 — division by zero safe when no attempts', () {
    engine.init(_makeContext());
    expect(engine.snapshot()['firstDartInSuccessRate'], isNull);
  });

  // ── Category G ─────────────────────────────────────────────────────────────

  test('G2 — two instances do not share state', () {
    final e2 = X01FirstDartInProjection();
    engine.init(_makeContext());
    e2.init(_makeContext());
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    expect(e2.snapshot()['inAttempts'], 0);
  });

  // ── Category H ─────────────────────────────────────────────────────────────

  test('H1 — 500 legs of in attempts processed without issue', () {
    engine.init(_makeContext());
    for (int i = 0; i < 500; i++) {
      engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: i * 3));
      engine.apply(_makeEvent(
          'DartThrown',
          {'player_id': 'p1', 'segment': i.isEven ? 'D20' : '20', 'score': 40},
          seq: i * 3 + 1));
      engine.apply(_makeEvent('LegCompleted', {'winner_player_id': 'p1'}, seq: i * 3 + 2));
    }
    expect(engine.snapshot()['inAttempts'], 500);
    expect(engine.snapshot()['inSuccesses'], 250);
  });

  // ── GS1/GS2/GS3 ───────────────────────────────────────────────────────────

  test('GS1 — descriptor declares only x01', () {
    engine.init(_makeContext());
    expect(engine.descriptor.supportedGameTypes, equals({GameType.x01}));
  });

  test('GS2 — cricket not in supported game types', () {
    engine.init(_makeContext());
    expect(engine.descriptor.supportedGameTypes.contains(GameType.cricket), isFalse);
  });

  test('GS3 — straight in context returns empty snapshot', () {
    engine.init(_makeContext(inStrategy: 'straight'));
    engine.apply(_makeEvent('TurnStarted', {'player_id': 'p1'}, seq: 1));
    engine.apply(_makeEvent('DartThrown', {'player_id': 'p1', 'segment': 'D20', 'score': 40}, seq: 2));
    expect(engine.snapshot(), isEmpty);
  });
}
